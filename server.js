...
if (process.env.YOUTUBE_COOKIES) {
  try {
    ...
    // Ensure the Netscape cookie file has the required header
    ...
    if (stat.size > 50) {
      ...
      console.warn('[cookies] Cookie file too small, ignoring');
    }
  } catch (e) {
    console.error('[cookies] Failed to write cookie file:', e.message);
  }
} else {
  console.log('[cookies] No YOUTUBE_COOKIES env variable found — proceeding without cookies');
}

function getCookieArgs() {
  if (cookiesAvailable && fs.existsSync(COOKIES_FILE)) return ['--cookies', COOKIES_FILE];
  return [];
}
...
// Retries without cookies if first attempt fails with a cookie-related error
async function ytDlpWithFallback(args) {
  try {
    return await ytDlp(args);
  } catch (err) {
    const isCookieError =
      /cookie/i.test(err.message) ||
      /HTTP Error 400/i.test(err.message) ||
      /Sign in to confirm/i.test(err.message) ||
      /This video is only available/i.test(err.message);

    if (isCookieError && cookiesAvailable) {
      console.warn('[yt-dlp] Cookie error detected, retrying without cookies...');
      // Strip --cookies <file> pair from args
      const argsNoCookies = [];
      for (let i = 0; i < args.length; i++) {
        if (args[i] === '--cookies') { i++; continue; }
        argsNoCookies.push(args[i]);
      }
      return await ytDlp(argsNoCookies);
    }
    throw err;
  }
}
...
app.get('/api/info', async (req, res) => {
  ...
  try {
    const json = await ytDlpWithFallback([
      '--dump-json',
      ...COMMON_ARGS,
      ...getCookieArgs(),
      parsed.href,
...
app.get('/api/download', async (req, res) => {
  ...
  try {
    ...
    await ytDlpWithFallback(dlArgs);
